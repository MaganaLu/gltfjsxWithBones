...
function parse(gltf, { fileName = 'model', ...options } = {}) {
    ...
    if (instanced) {
      ...
      if (obj.isInstancedMesh) {
        ...
        if (type === 'bone') {
          ...
          // Add ref for all bones when bones option is enabled
          if (options.bones) {
  ...
  // Collect all bones for ref gen
  const allBones = options.bones
    ? objects.filter((o) => o.isBone && o.__removed === undefined)
    : []
  const header = `/*
