...
function parse(gltf, { fileName = 'model', ...options } = {}) {
  ...
  function print(obj, silent = false) {
    ...
    const isRootBone = options.bones && type === 'bone' && !(obj.parent && obj.parent.isBone)
    ...
    if (instanced) {
      ...
      if (obj.isInstancedMesh) {
        ...
        if (type === 'bone') {
          result = `<primitive object={${node}} `
          if (isRootBone) {
            const refName = isVarName(obj.name) ? obj.name : obj.name.replace(/[^a-zA-Z0-9_]/g, '_')
            result += `ref={boneRefs.${refName}} `
          }
        }
        else result = `<${type} `
      }
  ...
  // Collect root bones for ref gen
  const rootBones = options.bones
    ? objects.filter((o) => o.isBone && !(o.parent && o.parent.isBone) && o.__removed === undefined)
    : []
  const header = `/*
  ...
  const result = `${options.types ? `\nimport * as THREE from 'three'` : ''}
        ...
        export ${options.exportdefault ? 'default' : ''} ${
          ...
          ${
            rootBones.length > 0
              ? `
          // Create refs for each root bone
          const boneRefs = {
            ${rootBones
              .map((bone) => {
                const refName = isVarName(bone.name) ? bone.name : bone.name.replace(/[^a-zA-Z0-9_]/g, '_')
                return `${refName}: ${options.types ? 'React.useRef<THREE.Bone>(null)' : 'React.useRef(null)'}`
              })
              .join(',\n            ')}
          }
